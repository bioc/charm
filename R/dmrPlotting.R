...
dmrPlot <- function(dmr=dmr, which.table=1:length(dmr$tabs), which.plot=1:30, legend.size=1, all.lines=TRUE, all.points=FALSE, colors.l, colors.p, outpath=".") {
    ...
    if(is.null(dmr$DD)) {
        if(missing(colors.p)) colors.p=2:length(unique(dmr$groups))
        if(missing(colors.l)) colors.l=2:length(unique(dmr$groups))        
        ...
        if(missing(colors.p)) colors.p = 2:ncol(dmr$sMD)
        if(missing(colors.l)) colors.l = 2:ncol(dmr$sMD)
...
dmrPlot_unpaired <- function(dmr=dmr, which.table=1:length(dmr$tabs), which.plot=1:30, legend.size=1, all.lines=TRUE, all.points=FALSE, colors.l, colors.p, outpath="."){

stopifnot(inherits(which.plot,"integer")|inherits(which.plot,"numeric"))
stopifnot(inherits(which.table,"integer")|inherits(which.table,"numeric"))
if(length(dmr$tabs)<length(unique(which.table))) stop("which.table specifies more comparisons than were made.")
...
for(object.i in which.table){
    message("Plotting table ",object.i)
    #################Make output table:###############################
    obj = tabs[[object.i]]
    #grs = strsplit(names(tabs)[object.i],"-")[[1]] #if this then no "-" allowed in group names
...
dmrPlot_paired <- function(dmr=dmr, which.table=1:length(dmr$tabs), which.plot=1:30, legend.size=1, all.lines=TRUE, all.points=FALSE, colors.l, colors.p, outpath="."){
...
sMD = dmr$sMD
FACS = dmr$groups
if(all.points)  stopifnot(length(colors.p)==ncol(sMD))
if(all.lines)   stopifnot(length(colors.l)==ncol(sMD))
if(!all.points) stopifnot(length(colors.p)%in%c(1, ncol(sMD)))
